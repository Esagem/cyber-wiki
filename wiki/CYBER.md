...
- **`_log.md`** — chronological, append-only. Prefix every entry with `## [YYYY-MM-DD] <op> | <context> | <short>` so it is greppable (`grep "^## \[" _log.md | tail -20`). Operations: `ingest`, `session`, `spec`, `lint`, `schema`, `write`, `delete`.
...
| `wiki_delete` | Permanently delete a page. Refuses by default unless the page looks like a test artifact. Pass `force=true` to delete real pages, but prefer setting `status: retired` in the page's front matter instead. |
...
**Important:** scope for `wiki_write` and `wiki_delete` is restricted to paths under the wiki root. Files at the repo root (e.g. `ONBOARDING.md` at the top level) are not reachable through the MCP tools — edit them locally or through the GitHub UI.
