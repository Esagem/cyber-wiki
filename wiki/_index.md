...
| [[synthesis/wiki-acceleration\|Wiki Acceleration — MCP Tool Additions]] | active | high | mcp, infrastructure, performance, tooling |
...
- **2026-04-24 (MCP tooling expansion)** — Approved spec for five new MCP tools to accelerate wiki interactions: `wiki_edit` (patch primitive, multi-edit transactional, refuses front matter), `wiki_read` with `section=` and `wiki_read_many` (batched reads, optional category shortcut, optional section filter), `wiki_status_set` (front matter mutations with schema validation), `wiki_log_tail` (recent log entries as structured data). Spec at [[synthesis/wiki-acceleration|wiki-acceleration]]; CYBER.md §9 rewritten to teach the LLM when to use each tool with explicit decision rules. Implementation pending; new tools become callable when the server ships them. Estimated 70% latency cut on lint-style and small-fix workflows.
