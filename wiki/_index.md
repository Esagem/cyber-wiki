...
- **2026-04-23 (probability_real removed)** — The fractional `probability_real` scoring axis was removed from the slice 1 spec after Eli flagged it was never approved. Priority is now three axes: severity × confidence × target_weight. False-positive doubt is expressed by committing to `status = false-positive`, not by a fractional downweight. Spec, glossary, vision, scope, users-and-jobs, architecture overview, open-questions, and leverage-analysis all updated.
