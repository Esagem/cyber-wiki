...
**Status: all criteria met as of 2026-04-26.** Slice 3 is shipped on `origin/main` at commit `422b8ef`. 275 tests pass (1 skipped, the pre-existing POSIX-only one); slice 1/2 surface preserved with type-name renames migrated through. End-to-end recursion verified via `scripts/run_slice3_demo.py` against `scripts/test_target_recurse.py` with the `linkfinder` example plugin: depth 0 cascade harvests 16 typed URLs, depth 1 fans out 15 deduped tasks, depth 2 exhausts. Lineage columns persist correctly; depth-0 scans have `parent_scan_id = NULL`, depth-1 scans parent on the depth-0 root scan. Plugin discovery integration confirmed: a real plugin file dropped in `CSAK_PLUGIN_DIR` registers a new type (`asn` parents `[network_block]`), registers a new tool, joins the routing graph live, and shows up correctly in `csak tools show httpx`. Six post-implementation deviations were surfaced by Claude Code and have been written back into this spec (network_block in accepts, conservative host recognizer, depth-0 dedup interaction, dedup-set seeding, doctor's same-tool-feedback handling, registry validation gating csak collect startup); plus one gap surfaced about plugin parser registration that was not in the original spec and is now documented under §Plugin discovery. Real-target use by Eli is the one remaining bullet — expected to follow naturally as slice 3 is exercised in normal cybersecurity work.
