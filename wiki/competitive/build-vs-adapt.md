...
- Cheaper than rewriting? **Probably not worth a literal copy.** Their parsers return DefectDojo's `Finding` object; ours needs to return CSAK's `Finding` entity with our severity/confidence/target-weight shape. The parsing *logic* transfers, but the output shape doesn't.
...
- Cheaper than rewriting? These are small enough that copying the data isn't really "copying code." **It's reference data, and CSAK needs its own anyway** because we're adding a confidence axis and target-weight axis that DefectDojo doesn't have.
- **Recommendation: use as a starting point for CSAK's own tables.** Cite DefectDojo as the source in the front matter of our severity-model spec.
...
- **Recommendation: adapt the template content as a starting point for CSAK's fix-it ticket library.** Attribute DefectDojo. Rewrite where our narrative style differs from theirs.
...
- **Recommendation: adapt these as documented defaults in CSAK's tool catalog.** These are closer to configuration than code.
...
- **Recommendation: accept as a slice 1 ingest format.** No code sharing required.
...
- **Recommendation: ignore.** We'll prototype LLM use in slice 1 against our own structured data (see the slice 1 spec's LLM section). Their approach might become useful reference later but isn't load-bearing.
...
| DefectDojo — CWE remediation templates | ✅ (content) | | |
| DefectDojo — Django web app | | | ✅ |
| reconFTW — orchestration pipeline | | ✅ | |
| reconFTW — tool invocation recipes | ✅ (config, not code) | | |
| reconFTW — `report/report.json` ingest | n/a (output parsing) | | |
...
1. **Write CSAK's code fresh.** Parsers, triage, dedup, reporting — all of it. The architectural win (clean data model, confidence + target-weight axes, narrative reports, CLI-first) requires a code structure that neither DefectDojo nor reconFTW is shaped for. Forking either means fighting their architecture forever.
...
3. **Adapt configuration with attribution.** reconFTW's specific tool invocation flags (Nuclei, Subfinder, httpx) are small, proven, and hard to reproduce from scratch. Document them in CSAK's tool catalog as defaults with a `# source: reconFTW v4.0` comment.
...
- **The data model mismatch is fundamental.** DefectDojo is Product → Engagement → Test → Finding. CSAK is Org → Target → Finding with separate Artifact and Report layers. Reshaping DefectDojo to match would touch every file.
...
- **The parts that matter most are the parts we'd rewrite anyway.** Triage (because we're adding confidence + target-weight), reports (because narrative fix-it tickets aren't DefectDojo's output shape), CLI (because DefectDojo doesn't have one), LLM integration (because it's gated behind Pro in DefectDojo).
...
## What changes in the slice 1 spec

This analysis supports (rather than challenges) the current slice 1 approach. Minor suggested additions:

- **Add a citation to DefectDojo's severity mapping tables** in whatever page ends up holding the triage translation tables (probably `specs/triage-model.md`).
- **Add a citation to reconFTW's tool invocation defaults** when we get to slice 2's tool catalog.
- **Create a `research/references.md` page** cataloging which external projects have contributed ideas or data to CSAK, so attribution is in one place instead of scattered.
